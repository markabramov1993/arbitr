...
SKILL_DIR = ROOT / "mermail-bounty-ops"
...
# Official Mermail authoring constraints for companion skills.
check(skill.startswith("---\n"), "SKILL.md must begin with YAML frontmatter")
parts = skill.split("---", 2)
check(len(parts) == 3, "SKILL.md frontmatter must have closing --- delimiter")
frontmatter = parts[1] if len(parts) == 3 else ""

# Only name/description/metadata are permitted as top-level frontmatter keys.
top_level_keys = []
for line in frontmatter.splitlines():
    if line and not line.startswith((" ", "\t", "#")) and ":" in line:
        top_level_keys.append(line.split(":", 1)[0].strip())
check(set(top_level_keys) == {"name", "description", "metadata"},
      f"frontmatter top-level keys must be exactly name, description, metadata; got {top_level_keys}")
...
check("metadata:" in frontmatter and "openclaw:" in frontmatter,
      "metadata.openclaw is required")
check("primaryEnv: MERMAIL_API_KEY" in frontmatter,
      "primaryEnv must be MERMAIL_API_KEY")
check(re.search(r"(?m)^\s*- MERMAIL_API_KEY\s*$", frontmatter) is not None,
      "metadata.openclaw.requires.env must include MERMAIL_API_KEY")
check("homepage: https://docs.mermail.app/ai/skills" in frontmatter,
      "Mermail skill homepage metadata is required")

line_count = len(skill.splitlines())
check(line_count <= 500, f"SKILL.md must be <= 500 lines; got {line_count}")
check("[tools.md](references/tools.md)" in skill,
      "SKILL.md must link references/tools.md")
check("[security.md](references/security.md)" in skill,
      "SKILL.md must link references/security.md")

# OpenAI metadata requirements from AUTHORING.md.
...
check('url: "https://console.mermail.app/mcp"' in openai,
      "OpenAI metadata must point at the hosted Mermail MCP endpoint")

# Safety invariants expected for skills interpreting economic/untrusted email.
lower_security = security.lower()
for phrase, explanation in [
    ("untrusted data", "email must be treated as untrusted data"),
    ("sender_authentication.status", "sender authentication guidance is required"),
    ("bounded reads", "bounded-read guidance is required"),
    ("otp", "OTP handling guidance is required"),
    ("magic link", "magic-link handling guidance is required"),
    ("wallet", "wallet/payment boundary guidance is required"),
]:
    check(phrase in lower_security, explanation)
check("approval" in lower_security and "external" in lower_security,
      "external effects must be tied to explicit approval")

# Core accounting invariant must stay visible in the main skill.
...
check("never report an opportunity amount as earned cash" in lower_skill,
      "SKILL.md must explicitly prevent nominal reward from being reported as cash")

# Tool-contract sanity checks.
for tool_name in (
    "list_mailboxes",
    "search_emails",
    "get_email",
...
# The authored package must not contain unresolved placeholder markers in core docs.
placeholder = "TO" + "DO"
...
# Security/dedup/accounting scenarios are machine-readable and complete.
try:
...
check(isinstance(scenarios, list) and len(scenarios) >= 6,
      "scenarios.json must contain at least 6 scenarios")
