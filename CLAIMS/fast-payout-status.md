...
2. **#1102 second/final item — 5 RTC requested:** mobile login does not validate the API key. Submitted Sep 10; no acceptance reply yet. Contributor cap is now full, so no additional #1102 claims.
3. **#13949 — 2 RTC:** README badge claim already sent; no duplicate.
4. **#1575 — 3–5 RTC canonical amount:** contributor-registry registration sent through the documented 403 email fallback; no reply yet.
5. **#1579 — 3 RTC:** contextual Elyan Labs mention in the pre-existing `arbitr` README; claim already sent; no reply yet.
6. **#100 — 2 RTC:** first Discovery Mode claim already sent; no reply yet.
7. **#16497 — 26 RTC requested as draft-acceptance tranches:** two Sep 4 articles, 13 RTC each; correction/reconciliation already sent.
8. **#16601 Type B — 15 RTC:** Proof-of-Antiquity YouTube script/storyboard package submitted Sep 4 and re-audited Sep 10.
9. **#16601 Type D — +8 RTC conditional:** syndication add-on submitted Sep 4; depends on acceptance of the backing article. Type C lane is occupied and is not forecast.

## #315 duplicate/funding checks performed before submission
- Gmail SENT search across the prior year found no earlier #315 / Human Funnel Stage 1 submission from this account.
- No incoming Sophia #315 adjudication was present before submission.
- Public #315 thread contains multiple recent email-fallback submissions explicitly accepted and queued at **30 RTC**, confirming that the email fallback is a real review route and that the title rate is the current rate.
- Submission therefore uses 30 RTC only and does not assert acceptance/payment.
